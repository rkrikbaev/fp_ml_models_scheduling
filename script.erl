...
trigger_tag( ModelControlTag )->
    ...
    #{<<"trigger">> := TriggerValue0} = fp_db:read_fields( fp_db:open( GlobalModelControlTag ),[<<"trigger">>]),

    % create and update "model point" field of model tag
    ...
    % update global tag value if additional logs need
    ...
    try
        ...
        % check if all amjor field are not empty
        ...
        % check model configuration map field
        % check major fields are present, not empty and have values specific type
        ...
        % incriment trigger value causes send request to a model
        case TaskState of
            <<"SUCCESS">> ->
